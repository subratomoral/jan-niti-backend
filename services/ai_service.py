...
def analyze_issue_text(raw_text: Optional[str] = "") -> TextAnalysisResponse:
    """Primary entry point: safely handles None/str, filters noise/abc inputs immediately."""
    clean_text = str(raw_text or "").strip()

    # STRICT NOISE / ABC CHECK: Agar input chota hai ya meaningless hai
    if len(clean_text) < 4 or clean_text.lower() in [
        "abc",
        "hi",
        "hello",
        "test",
        ...
        return TextAnalysisResponse(
            ...
            priority_signal="low",
            confidence=0.10,
            ai_observation="Input filtered as non-civic noise or meaningless characters.",
            analysis_mode="demo",
        )

    # Baaki ka purana LLM aur Heuristic code yahan rahega...
